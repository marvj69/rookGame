...
function estimateStaticBidCeiling(hand) {
...
function getBidSampleDeck(hand) {
  const heldIds = cardIdSet(hand);
  return buildDeck().filter((card) => !heldIds.has(card.id));
}

function estimatePotentialBidFromSampledFullHand(fullHand) {
  const best = COLORS.reduce(
    (bestTrump, trump) => {
      const discardRule = getKittyDiscardRule(fullHand, trump);
      if (!discardRule.canSatisfy) return bestTrump;

      const evaluation = evaluateHandForTrump(fullHand, trump);
      const discardPressure = discardRule.requiredTrumpPointCount * 8;
      const adjustedScore = evaluation.score - discardPressure;
      return adjustedScore > bestTrump.score ? { trump, score: adjustedScore, pointTotal: evaluation.pointTotal } : bestTrump;
    },
    { trump: COLORS[0], score: -Infinity, pointTotal: 0 },
  );

  if (!Number.isFinite(best.score)) return 95;

  const rawBid = 62 + best.score * 0.5 + best.pointTotal * 0.05;
  return clamp(roundDownToFive(rawBid), 95, MAX_BID);
}

function getBidContextAdjustment(game, playerId, nextBid, sampledCeiling) {
  const playerTeam = teamForPlayer(playerId);
  const currentBidder = game.bidInfo?.bidder ?? null;
  const currentBidderTeam = currentBidder === null ? null : teamForPlayer(currentBidder);
  const scores = game.scores ?? { us: 0, them: 0 };
  const opponent = otherTeam(playerTeam);
  const scoreDiff = (scores[playerTeam] ?? 0) - (scores[opponent] ?? 0);
  const leaderNearGame = Math.max(scores.us ?? 0, scores.them ?? 0) >= TARGET_SCORE - 120;
  const playerIsDealer = game.dealer === playerId;
  const turnsUntilDealer = (game.dealer - playerId + 4) % 4;
  let adjustment = 0;

  if (currentBidderTeam === playerTeam) {
    adjustment -= sampledCeiling >= nextBid + 25 ? 5 : 18;
  } else if (currentBidderTeam !== null) {
    adjustment += scoreDiff < -120 ? 7 : scoreDiff < -60 ? 4 : 0;
    adjustment += leaderNearGame && (scores[opponent] ?? 0) > (scores[playerTeam] ?? 0) ? 4 : 0;
  }

  if (scoreDiff > 140) adjustment -= 8;
  if (scoreDiff > 80) adjustment -= 4;
  if (playerIsDealer) adjustment += currentBidder === null ? -5 : 3;
  if (turnsUntilDealer === 3 && currentBidder === null) adjustment -= 3;

  return adjustment;
}

function estimateBidEv(hand, game, playerId) {
  const cacheKey = `${playerId}|${getOrderedHandSignature(hand)}`;
  const cached = bidEvCache.get(cacheKey);
  if (cached) return cached;

  const staticEstimate = estimateStaticBidCeiling(hand);
  const deck = getBidSampleDeck(hand);
  const sampleCeilings = [];
  const seedBase =
    13007 +
    playerId * 313 +
    hand.reduce((sum, card) => sum + card.id * 23, 0);

  for (let sampleIndex = 0; sampleIndex < BID_KITTY_SAMPLE_COUNT; sampleIndex += 1) {
    const sampledKitty = shuffleCards(deck, seedBase + sampleIndex * 7919).slice(0, DISCARD_COUNT);
    sampleCeilings.push(estimatePotentialBidFromSampledFullHand(sortHand([...hand, ...sampledKitty])));
  }

  const averageCeiling =
    sampleCeilings.reduce((sum, ceiling) => sum + ceiling, 0) / Math.max(1, sampleCeilings.length);
  const variance =
    sampleCeilings.reduce((sum, ceiling) => sum + (ceiling - averageCeiling) ** 2, 0) /
    Math.max(1, sampleCeilings.length);
  const riskPenalty = Math.sqrt(variance) * 0.25;
  const sampledCeiling = clamp(roundDownToFive(averageCeiling - riskPenalty), 95, MAX_BID);
  const blendedCeiling = clamp(
    roundDownToFive(staticEstimate.ceiling * 0.55 + sampledCeiling * 0.45),
    95,
    MAX_BID,
  );

  const estimate = {
    trump: staticEstimate.trump,
    staticCeiling: staticEstimate.ceiling,
    sampledCeiling,
    ceiling: blendedCeiling,
    strength: staticEstimate.strength,
  };

  rememberCacheValue(bidEvCache, cacheKey, estimate, BID_EV_CACHE_LIMIT);
  return estimate;
}

function estimateBidCeiling(hand, game, playerId, nextBid) {
  const estimate = estimateBidEv(hand, game, playerId);
  const adjustment = getBidContextAdjustment(game, playerId, nextBid, estimate.sampledCeiling);

  return {
    ...estimate,
    ceiling: clamp(roundDownToFive(estimate.ceiling + adjustment), 95, MAX_BID),
  };
}
...
function getKittyPlanCacheKey(handSignature, context) {
  const game = context?.game;
  const playerId = context?.playerId ?? context?.winner ?? game?.bidInfo?.bidder ?? game?.dealer ?? "none";

  if (!game || context?.rollout === false) return handSignature;

  return [
    handSignature,
    playerId,
    game.dealer ?? "d",
    game.bidInfo?.bidder ?? "b",
    game.bidInfo?.highBid ?? "h",
    game.scores?.us ?? 0,
    game.scores?.them ?? 0,
  ].join("|");
}
...
function addRankedKittyPlan(plans, plan, limit) {
  plans.push(plan);
  plans.sort((a, b) => b.score - a.score);
  if (plans.length > limit) plans.pop();
}

function getRankedHeuristicKittyPlans(fullHand, limit = 1) {
...
function chooseBestHeuristicKittyPlan(fullHand) {
...
function createKittyRolloutGame(plan, context, sampleIndex) {
  ...
  return {
    ...
    hands,
    kitty: [],
    kittyPoints: plan.discards.reduce((sum, card) => sum + card.value, 0),
    trump: plan.trump,
    bidInfo: {
      ...(sourceGame.bidInfo ?? {}),
      active: false,
...
function advanceRolloutTurn(game) {
  game.currentTurn = (game.currentTurn + 1) % 4;
}

function playRolloutCard(game, playerId, card) {
  const hand = game.hands[playerId];
  const leadColor = getLeadColor(game.currentTrick, game.trump);

  if (!card || !isValidMove(card, hand, leadColor, game.trump)) return false;

  const cardIndex = hand.findIndex((heldCard) => heldCard.id === card.id);
  if (cardIndex < 0) return false;

  const [playedCard] = hand.splice(cardIndex, 1);
  game.currentTrick.push({ pid: playerId, card: playedCard });
  advanceRolloutTurn(game);
  return true;
}

function resolveRolloutTrick(game) {
  const leadColor = getLeadColor(game.currentTrick, game.trump);
  let winner = game.currentTrick[0].pid;
  let bestPower = getCardPower(game.currentTrick[0].card, game.trump, leadColor);
  const points = game.currentTrick.reduce((sum, play) => sum + play.card.value, 0);
  ...
  game.pointsTaken[teamForPlayer(winner)] += points;
...
function simulateKittyPlanRound(plan, context, sampleIndex) {
  ...
  while (game.hands[playerId].length > 0 && guard < 80) {
    guard += 1;
    const nextPlayerId = game.currentTurn;
    const choice = chooseBotPlay(game, nextPlayerId);

    if (!playRolloutCard(game, nextPlayerId, choice)) return null;
    if (game.currentTrick.length === 4) resolveRolloutTrick(game);
  }
...
function canUseKittyRolloutContext(context) {
  return Boolean(context?.game && Number.isInteger(context?.playerId ?? context?.winner ?? context.game.bidInfo?.bidder));
}

export function chooseBotKittyPlan(fullHand, context = null) {
  const handSignature = getOrderedHandSignature(fullHand);
  const cacheKey = getKittyPlanCacheKey(handSignature, context);
  const cachedPlan = kittyPlanCache.get(cacheKey);
  if (cachedPlan) return materializeKittyPlan(fullHand, cachedPlan);

  const rankedPlans = getRankedHeuristicKittyPlans(fullHand, KITTY_ROLLOUT_CANDIDATES);
  let plan = rankedPlans[0] || createFallbackKittyPlan(fullHand);

  if (canUseKittyRolloutContext(context) && rankedPlans.length > 1) {
    plan = [...rankedPlans]
      .map((candidate) => ({
        ...candidate,
      ...
      .sort((a, b) => {
        ...
        if (rolloutDiff !== 0) return rolloutDiff;
  ...
  rememberKittyPlan(cacheKey, plan);
  return plan;
}
